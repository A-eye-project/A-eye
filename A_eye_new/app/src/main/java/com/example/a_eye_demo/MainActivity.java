...
import com.example.a_eye_demo.Audio.Recording;
import com.example.a_eye_demo.Back.MyService;
...
import com.example.a_eye_demo.Support.Myapp;
...
public class MainActivity extends AppCompatActivity {
    ...
    private boolean Fun_state = false;
    public static boolean activity_die = false;
    //지연시간 변수
    private Handler mHandler = new Handler();;

    public static starting start;
    ...
    private boolean service_not_start;
    ...
    protected void onResume() { // 요부분 없으면 권한 설정에서 다 깨진다.
        ...
        if (getApplication() instanceof Myapp) {
            if (((Myapp)getApplication()).isReturnedForground()) {
                Log.i("app","isForeground");
            }
            else Log.i("app","Background");
        }
        else{
            Log.i("app","?");
        }
    ...
    public void start_Functions(){
        if (getApplication() instanceof Myapp) {
            if (((Myapp)getApplication()).isReturnedForground() == false) {
        ...
        mHandler.postDelayed(new Runnable(){ // 녹음 하는 시간 지연
            ...
            public void run(){
                ...
                if(status == 1){
                    ...
                    if(status == -2){
                        Log.i("now","No response from server for 20 secs");
                    }
                    else{
                        Log.i("now","Interrupted");
                    }
                }
                th.interrupt();
            }
    ...
    public void onBackPressed() {
        ...
        if (System.currentTimeMillis() > backKeyPressedTime + 2500) {
            ...
            makeToast("뒤로 가기 버튼을 한 번 더 누르시면 백그라운드 상태로 전환됩니다.");
            return;
        }
        // 마지막으로 뒤로 가기 버튼을 눌렀던 시간에 2.5초를 더해 현재 시간과 비교 후
        // 마지막으로 뒤로 가기 버튼을 눌렀던 시간이 2.5초가 지나지 않았으면 종료 + 포그라운드 서비스 실행
        if (System.currentTimeMillis() <= backKeyPressedTime + 2500) {
            moveTaskToBack(false);
        }
    }
    public void startcommand(){
        Thread r_th = new Thread(new Runnable() {
            @Override
            public void run() {
                start.onSetup();
            }
    ...
    public void ImageUploadToServer(){
        ...
        class starting implements RecognitionListener {
            // 활성 키워드
            private final String KWS_SEARCH = "wakeup";
            private final String KEYPHRASE = "adam";
            ...
            public void onSetup() {
                ...
                new starting.setupTask(this).execute();
            }

            private class setupTask extends AsyncTask<Void, Void, Exception> {
                WeakReference<starting> activityReference;

                setupTask(starting activity) {
            ...
            public void call_main(){
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB)
                {
                    ...
                    for (int i = 0; i < recentTasks.size(); i++)
                    {
                        ...
                        if (recentTasks.get(i).baseIntent.getComponent().getPackageName().equals("com.example.a_eye_demo")) {
